...
router.post('/cart/products',async(req,res)=>{
    ...
    if(!req.session.cartId){
         ...
         cart = await cartsRepository.getOn(req.session.cartId);
    }
...
router.get('/cart',async(req,res)=>{
    ...
    const cart = await cartsRepository.getOn(req.session.cartId);

    for(let item of cart.items){
        const product = await productsRepositories.getOn(item.id);
